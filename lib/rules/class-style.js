...
module.exports.lint = function (classes, opts) {
  const format = opts[this.name] || opts['id-class-style'];
  const ignore_class = classes.ignore_class;

  return proc.chimeraClean(classes).filter(function(c, i) {
    return !(ignore_class[i] || format.test(c));
  }).map(function(c, i) {
    return new Issue('E011', classes.lineCol, { attribute: 'class', format: format.desc, value: c });
  });
}
